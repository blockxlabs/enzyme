import React, { Component } from 'react';
import TextField from '@material-ui/core/TextField';
import './styles.css';
import { withStyles } from '@material-ui/core';

const styles = () => ({
  container: {
    display: 'flex',
    flexWrap: 'wrap',
  },
  textField: {
    width: '320px',
  },
  inputRoot: {
    backgroundColor: 'rgba(0, 0, 0, 0.06)',
    borderRadius: '4px',
    paddingTop: '18px',
    paddingLeft: '18px',
    paddingRight: '18px',
    color: 'rgba(0, 0, 0, 0.87)',
    fontFamily: 'Roboto-Regular',
    fontSize: '16px',
  },
  input: {
    '&::placeholder': {
      color: '#000000',
      opacity: '0.6',
      fontSize: '16px',
      fontFamily: 'Roboto-Regular',
    },
  },
  inputUnderline: {
    '&::before': { border: 'none' },
    '&::after': { borderBottom: '2px solid rgba(215, 95, 160, 1)' },
  },
  inputErrorUnderline: {
    '&$inputError': {
      '&::before': { border: 'none' },
      '&::after': { borderBottom: '2px solid rgba(0, 0, 0, 0.3)' },
    },
  },
  inputError: {},
  helperText: {
    paddingLeft: '16px',
    '&$helperTextError': {
      color: 'rgba(176, 0, 32, 1)',
      fontFamily: 'Roboto-Regular',
      fontSize: '11px',
    },
  },
  helperTextError: {},
});

class ImportSeedPhrase extends Component {
  constructor(props) {
    super(props);
    this.seedWordsInput = React.createRef();
  }

  componentDidMount() {
    this.props.handleSeedWordImportOnMount();
  }

  render() {
    const {
      classes, onChange, seedWords, isError, errorMessage, onBlur
    } = this.props;
    return (
      <div className="import-seed-phrase-container">
        <div className="import-seed-phrase-title">Import Seed Phrase</div>
        <div className="import-seed-phrase-description">
          This seed phrase is used to generate your first account. Make sure it&#39;;s saved
          somewhere safe and don&#39;t share it.
        </div>
        <div className="import-seed-phrase-input">
          <TextField
            inputRef={input => {
              this.seedWordsInput = input;
            }}
            placeholder="Type or paste your seed phrase..."
            multiline
            error={isError}
            onChange={onChange('importedSeedPhrase')}
            onBlur={onBlur(this.seedWordsInput)}
            value={seedWords}
            rows="5"
            rowsMax="5"
            className={classes.textField}
            helperText={errorMessage}
            FormHelperTextProps={{
              classes: {
                root: classes.helperText,
                error: classes.helperTextError,
              },
            }}
            InputProps={{
              classes: {
                root: classes.inputRoot,
                input: classes.input,
                underline: classes.inputUnderline,
                error: classes.inputError,
              },
            }}
          />
        </div>
      </div>
    );
  }
}

export default withStyles(styles)(ImportSeedPhrase);
